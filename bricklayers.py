...
# Interlocking Layers Post-Processing Script for PrusaSlicer, OrcaSlicer, BambuStudio and Ultimaker Cura
#
...
class GCodeFeature:
    ...
    #
    # BrickLayers was originally written to work with Prusa‑ and OrcaSlicer G‑code
    # which tag perimeters using the labels “Perimeter”, “Inner wall”, “External perimeter”
    # and so on.  Ultimaker Cura uses a different naming scheme in its `;TYPE:`
    # comments (e.g. `WALL‑INNER`, `WALL‑OUTER`, `WALL‑OVERHANG`).  To make the
    # parser recognise Cura’s labels as well, we include those synonyms here.
    # When a new Cura release adds more `;TYPE:` labels, they can be appended
    # to these sets to improve compatibility.  See https://community.ultimaker.com
    # for a list of Cura’s known `;TYPE` values【256029318565517†L110-L116】.
    DEF_INNERPERIMETERS = {
        "Perimeter",        # Prusa/Orca
        "Inner wall",       # Prusa/Orca
        "WALL-INNER",       # Cura
        "WALL INNER",       # Sometimes Cura uses space separator
        "WALL_INNER",       # Generic variant
    }
    # Outer perimeters / external walls
    DEF_OUTERPERIMETERS = {
        "External perimeter",  # Prusa/Orca
        "Outer wall",          # Prusa/Orca
        "WALL-OUTER",          # Cura
        "WALL OUTER",          # Cura with space
        "WALL_OUTER",          # Generic variant
    }
    # Overhang walls.  Cura sometimes tags special walls such as overhangs.
    DEF_OVERHANGPERIMETERS = {
        "Overhang wall",    # OrcaSlicer
        "WALL-OVERHANG",    # Cura equivalent if present
        "WALL OVERHANG",    # variant with space
        "WALL_OVERHANG",    # generic variant
    }
    DEF_WIPESTARTS   = {";WIPE_START", "; WIPE_START"}
    DEF_WIPEENDS     = {";WIPE_END",   "; WIPE_END"}
    #
    # Known layer change markers.  PrusaSlicer and OrcaSlicer emit
    # `;LAYER_CHANGE` or `; CHANGE_LAYER`.  Ultimaker Cura annotates
    # each new layer with `;LAYER:<index>` where `<index>` is a zero‐based
    # layer number【471852490826798†L68-L94】.  We treat any line beginning
    # with that prefix as a layer change so that BrickLayers updates its
    # internal layer counter appropriately.
    DEF_LAYERCHANGES = {";LAYER_CHANGE", "; CHANGE_LAYER"}
    # Additional layer change prefixes for slicers that embed the layer
    # number after a colon (e.g. Cura).  Strings in this tuple are
    # compared with `str.startswith()` rather than strict equality.
    DEF_LAYERCHANGE_PREFIXES = (";LAYER:",)
    #
    # Known layer height markers.  Slic3r/Orca use `;HEIGHT:` or
    # `; LAYER_HEIGHT:` to indicate the nominal layer height.
    # Cura writes the human‑readable phrase `;Layer height: <value>` in
    # the file header【255174154358426†L118-L163】.  Add that prefix here so
    # that the height is captured when present.
    DEF_LAYER_HEIGHTS = (";HEIGHT:", "; LAYER_HEIGHT: ", ";Layer height:")
    # Known Z height markers.  Slic3r/Orca use `;Z:` or `; Z_HEIGHT:`
    # to record the absolute Z for each layer.  Cura does not emit
    # equivalent tags but encodes the Z height in the following G0/G1
    # command instead【471852490826798†L68-L87】, so we leave this unchanged.
    DEF_LAYER_ZS      = (";Z:", "; Z_HEIGHT: ")          # tupple, for line.startswith

    # Start/stop object delimiters.  Slic3r/Orca emit `; printing object` comments
    # when printing begins and `; stop printing object` when the object finishes.
    # Ultimaker Cura uses a different scheme: it marks the start of a mesh with
    # `;MESH:<filename>` and signals the end of printable geometry with
    # `;MESH:NONMESH` (or occasionally `;MESH:NOMESH`)【688603851195412†L150-L168】.
    # To support Cura, include `;MESH:` as a start prefix and the NONMESH/NOMESH
    # markers as stop prefixes.  The parser will extract the filename after
    # `;MESH:` as the object name and will treat `;MESH:NONMESH`/`;MESH:NOMESH`
    # as the end of the current object.
    DEF_START_PRINTING_OBJECTS = (
        "; printing object ",        # Prusa/Orca start marker
        "; start printing object, ", # BambuStudio variant
        ";MESH:",                    # Cura start marker (object name follows)
    )
    DEF_STOP_PRINTING_OBJECTS  = (
        "; stop printing object ",    # Prusa/Orca stop marker
        "; stop printing object, ",   # BambuStudio variant
        ";MESH:NONMESH",             # Cura end‑of‑mesh marker
        ";MESH:NOMESH",              # Cura variant (typo in some versions)
    )
    ...
    def parse_gcode_line(self, line):
        ...
        elif (strippedline in self.DEF_LAYERCHANGES) or any(
            strippedline.startswith(pref) for pref in self.DEF_LAYERCHANGE_PREFIXES
        ):
            """Handle layer change comments.

            Slic3r-derived slicers emit `;LAYER_CHANGE` or `; CHANGE_LAYER`,
            whereas Cura uses `;LAYER:<index>`【471852490826798†L68-L94】.  When a
            line matches any of the known prefixes, update the layer counter
            accordingly, reset feature flags, and remember the flavour of the
            comment so it can be reinserted later.
            """
            # Capture the original layer-change line flavour on first use
            if self.const_layer_change is None:
                if strippedline in self.DEF_LAYERCHANGES:
                    # Preserve the exact string with newline to replicate later for
                    # Slic3r/Orca.  We store the whole line including the newline.
                    type(self).const_layer_change = line
                else:
                    # For Cura's `;LAYER:<index>` style, capture only the prefix
                    # up to and including the colon.  This allows us to
                    # reconstruct layer comments with the appropriate index later
                    # when emitting new layer headers.  We intentionally do not
                    # append the numeric suffix or a newline here so that the
                    # output code can supply the current layer number and newline
                    # as needed.
                    prefix = None
                    for pref in self.DEF_LAYERCHANGE_PREFIXES:
                        if strippedline.startswith(pref):
                            prefix = pref
                            break
                    if prefix is None:
                        prefix = ";LAYER:"
                    type(self).const_layer_change = prefix

            # Mark that we have encountered a new layer so other parts of
            # BrickLayers can emit appropriate header comments
            self.layer_change = True
            # Reset perimeter flags – note: some slicers continue the previous
            # perimeter type onto the next layer, but BrickLayers clears these
            # states to simplify processing.
            self.internal_perimeter = False
            self.external_perimeter = False
            self.overhang_perimeter = False
            # Update the internal layer counter: for Cura we respect the
            # numerical index if present; otherwise increment by one.
            if any(strippedline.startswith(pref) for pref in self.DEF_LAYERCHANGE_PREFIXES):
                # Extract the number after the colon and convert to int, if possible
                try:
                    # split once on ':' and ignore trailing comments after the number
                    number_str = strippedline.split(":", 1)[1]
                    # The number may be followed by whitespace or additional comments
                    number_str = number_str.strip().split()[0]
                    self.layer = int(float(number_str))
                except Exception:
                    self.layer += 1
            else:
                # For slicers that do not include the layer number, just increment
                self.layer += 1
            # Allow capturing of the layer height again for the new layer
            self.capture_height = True


        # Capture Z height markers, e.g. `;Z:` or `; Z_HEIGHT:`.  Loop
        # through the known prefixes so that we also record which form
        # appears in the input.  Cura does not emit such tags, so
        # `const_layer_z` may remain `None` and will be filled in later.
        elif any(strippedline.startswith(prefix) for prefix in self.DEF_LAYER_ZS):
        ...
        # Capture layer height markers, e.g. `;HEIGHT:0.2`,
        # `; LAYER_HEIGHT: 0.20` or Cura's `;Layer height: 0.2`
        #【255174154358426†L118-L163】.  We only update the height once per
        # layer (controlled by `self.capture_height`) to avoid overwriting
        # with redundant values.
        elif any(strippedline.startswith(prefix) for prefix in self.DEF_LAYER_HEIGHTS):
            if self.capture_height:
                for prefix in self.DEF_LAYER_HEIGHTS:
                    if strippedline.startswith(prefix):
                        try:
                            # Extract the numeric portion after the prefix and
                            # convert to float.  The value may be followed by
                            # whitespace or additional comments.
                            value_str = strippedline[len(prefix):].strip().split()[0]
                            self.height = float(value_str)
                        except Exception:
                            pass
                    ...
                    for line_index, deffered_line in enumerate(deffered_loop):
                        ...
                        if is_first_perimeter and is_first_loop and is_first_line:
                            ...
                            if feature.current_object is not None:
                                # When using Cura-style MESH comments the stop
                            ...
                            # Emit a layer-change comment.  For slicers like
                            # Prusa/Orca we re‑use the captured line verbatim.  For
                            # Cura we captured only the `;LAYER:` prefix, so append
                            # the current layer index and a newline to produce
                            # `;LAYER:<index>`【471852490826798†L68-L94】.
                            if feature.const_layer_change.startswith(";LAYER:"):
                                buffer.append(from_gcode(f";LAYER:{feature.layer}\n"))
                            else:
                                buffer.append(from_gcode(feature.const_layer_change)) # ex: ;LAYER_CHANGE
                            # Append a Z height comment only if the original G-code
                            # contained one.  Cura does not emit `;Z:` or
                            # `; Z_HEIGHT:` markers【471852490826798†L68-L87】, in which case
                            # `feature.const_layer_z` will be None and we skip
                            # inserting this comment.  Otherwise we use the stored
                            # prefix with the computed target Z.
                            if feature.const_layer_z is not None:
                                buffer.append(from_gcode(f"{feature.const_layer_z}{target_z_formated}\n"))  # e.g. ;Z:2.4
                            # Some slicers (e.g. Orca/Bambu) include a bare `;Z` line for
                            # preview; retain it only if `feature.const_layer_z` was set.
                            if feature.const_layer_z is not None:
                                buffer.append(from_gcode(f";{target_z_formated}\n"))
                            # Append a layer-height comment using the stored prefix (e.g.
                            # `;HEIGHT:` or `;Layer height:`).  For Cura this will be
                            # `;Layer height:` and ensures consistent preview of the
                            # inserted half-layer.
                            ...
                            # Append a width comment only if one was detected in
                            # the input G-code.  Cura does not emit `;WIDTH:`
                            # lines, so `simulator.const_width` will be None.  In
                            # that case we skip this comment to avoid emitting
                            # `None0.00`【688603851195412†L150-L168】.
    ...
    def process_gcode(self, gcode_stream):
        ...
        for line_number, line in enumerate(gcode_stream, start=1):
            ...
            if feature.justleft_internal_perimeter or (layer_changed_during_internal_perimeter):
                ...
                if len(group_perimeter) > 0:
                    ...
                    if len(kept_loops) > 0:
                        ...
                        # Only append a width comment when the slicer provided a
                        # width prefix.  Cura omits `;WIDTH:` comments, so
                        # `simulator.const_width` may be None【688603851195412†L150-L168】.  In that case we
                        # skip emitting the width line to avoid inserting `None0.00`.
                        if simulator.const_width is not None:
                            buffer_lines.append(from_gcode(f"{simulator.const_width}{myline.previous.width}\n"))   # For the Preview
                        # Emit the layer-height comment using the stored prefix.  This
                        # is always available for Orca/Prusa; for Cura it will be
                        # `;Layer height:`.
            ...
            if feature.justgotinside_external_perimeter: # SURE: WITHOUT this width, the preview gets very ugly from continuing with wrong widths
                # If the slicer provided a width comment, replicate it.  Cura
                # omits `;WIDTH:` so const_width will be None; skip to avoid
                # `None<value>` lines.
                if simulator.const_width is not None:
