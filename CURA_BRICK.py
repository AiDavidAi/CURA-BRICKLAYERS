#!/usr/bin/env python3
__version__ = "v0.2.1-10-g6409588-cura-only"  # Cura-only version

# Brick Layers by Geek Detour - Cura Edition
# Interlocking Layers Post-Processing Script for Cura
#
...
#
#  - Cura: Works with both "Classic" and "Arachne" wall generators
#          Wall ordering should be set to "Inner to Outer" for best results
#
#
# About Brick Layers:
...
class GCodeFeature:
    """
    GCodeFeature: A state tracker for parsing Cura G-code and identifying print features.

    This class analyzes Cura G-code lines to detect and classify different print features, such as 
    internal/external perimeters, layer changes, and object identification. It maintains 
    ...
    - Identifies whether a segment is an **internal or external perimeter**.
    - Recognizes **mesh/object changes** when switching between printed objects.
    - Efficiently captures **layer height information** while avoiding redundant updates.

    ### Parsing Behavior:
    - G-code comments like `;TYPE:WALL-INNER` define different print features.
    - Special markers such as `;LAYER:` and `;Z:` update layer state.
    - Mesh identification through `;MESH:` comments.
    """
    ...
    # Cura-specific definitions
    DEF_TYPES = (";TYPE:",)  # Cura uses ;TYPE:
    DEF_INNERPERIMETERS = {"WALL-INNER"}  # Cura's inner wall designation
    DEF_OUTERPERIMETERS = {"WALL-OUTER"}  # Cura's outer wall designation
    DEF_OVERHANGPERIMETERS = set()  # Cura doesn't have specific overhang walls
    
    # Cura doesn't have built-in wipe commands, so we'll create them when needed
    DEF_WIPESTARTS = {";WIPE_START"}  # We'll generate these
    DEF_WIPEENDS = {";WIPE_END"}      # We'll generate these
    
    # Cura layer change format
    DEF_LAYERCHANGES = set()  # Cura uses ;LAYER: instead
    DEF_LAYER_HEIGHTS = (";HEIGHT:",)
    DEF_LAYER_ZS = (";Z:",)
    
    # Cura object/mesh identification
    DEF_START_PRINTING_OBJECTS = (";MESH:",)
    DEF_STOP_PRINTING_OBJECTS = set()  # We'll generate stop commands
    ...
    def parse_gcode_line(self, line):
        ...
        # Handle Cura layer changes: ;LAYER:123
        if strippedline.startswith(";LAYER:"):
            ...
            try:
                layer_num = int(strippedline.split(":")[1])
        ...
        # Handle feature types: ;TYPE:WALL-INNER, ;TYPE:WALL-OUTER, etc.
        elif strippedline.startswith(";TYPE:"):
            old_type = self.current_type
            self.just_changed_type = True
            
            new_type = strippedline[6:].strip()  # Remove ";TYPE:"
            
            if new_type in self.DEF_INNERPERIMETERS:
                if self.internal_perimeter_type is None:
                    type(self).internal_perimeter_type = line
                self.current_type = self.SANE_INNERPERIMETER
                ...
                self.current_type = self.SANE_OUTERPERIMETER
        ...
        # Handle Cura mesh identification: ;MESH:ObjectName
        elif strippedline.startswith(";MESH:"):
            object_name = strippedline[6:].strip()  # Remove ";MESH:"
            if self.const_printingobject_start is None:
                type(self).const_printingobject_start = ";MESH:"
            self.current_object = ObjectEntry.entry_from_name(object_name)
        
        # Handle wipe commands (generated by us)
        elif strippedline in self.DEF_WIPESTARTS:
            self.wiping = True
            if self.const_wipe_start is None:
                type(self).const_wipe_start = ";WIPE_START\n"
        
        elif strippedline in self.DEF_WIPEENDS:
            self.wipe_willfinish = True
            if self.const_wipe_end is None:
                type(self).const_wipe_end = ";WIPE_END\n"
        
        # Handle height information
        elif strippedline.startswith(";HEIGHT:"):
            if self.capture_height:
                try:
                    self.height = float(strippedline[8:])  # Remove ";HEIGHT:"
                    self.capture_height = False
                except ValueError:
                    pass
                if self.const_layer_height is None:
                    type(self).const_layer_height = ";HEIGHT:"
        
        # Handle Z information if present
        elif strippedline.startswith(";Z:"):
            try:
                self.z = float(strippedline[3:])  # Remove ";Z:"
            except ValueError:
                pass
            if self.const_layer_z is None:
                type(self).const_layer_z = ";Z:"
...
class GCodeSimulator:
    """
    GCodeSimulator: A lightweight G-code state tracker for Cura.
    ...
    DEF_WIDTHS = (";WIDTH:",)  # Cura might not use width comments as much
...
class BrickLayersProcessor:
    """
    BrickLayersProcessor: G-Code Post-Processor for Brick Layering with Cura.

    This class modifies Cura G-code to implement the Brick Layers technique, improving part 
    ...
    - Detects and handles layer changes (`;LAYER:`).
    ...
    - Captures object changes for Cancel Objects (`; MESH:`).
    ...
    def __init__(self, extrusion_global_multiplier: float = 1.05, start_at_layer: int = 3, layers_to_ignore = None, verbosity: int = 0, progress_callback: Optional[Callable[[dict], None]] = None):
        ...
        self.cura_retraction_detected = False
        self.cura_wipe_speed = 0
    ...
    def detect_cura_retraction_settings(self, simulator, feature):
        """
        Detect retraction settings from Cura G-code since it doesn't have explicit wipe commands
        """
        if not self.cura_retraction_detected:
            # Look for retraction patterns in the G-code
            if simulator.is_retracting and simulator.retraction_length == 0:
                # First retraction detected
                simulator.retraction_length = abs(simulator.e - getattr(simulator, 'previous_e', 0))
                simulator.retraction_speed = simulator.f
                self.cura_retraction_detected = True
                    ...
                    for line_index, deffered_line in enumerate(deffered_loop):
                        ...
                        if is_first_perimeter and is_first_loop and is_first_line:
                            ...
                            if feature.current_object is not None:
                                buffer.append(from_gcode(f"; stop printing mesh {feature.current_object.name}\n"))
                            ...
                            buffer.append(from_gcode(f";Z:{target_z_formated}\n"))
                            buffer.append(from_gcode(f";HEIGHT:{feature.height:.2f}\n"))
                            ...
                            buffer.append(from_gcode(";TYPE:WALL-INNER\n"))  # Cura internal perimeter type
                            if simulator.const_width:
                        ...
                        if current_object is not deffered_line.object:
                            ...
                            if current_object is not None:
                                buffer.append(from_gcode(f"; stop printing mesh {current_object.name}\n"))
                            ...
                            if deffered_line.object:
                                buffer.append(from_gcode(f";MESH:{deffered_line.object.name}\n"))
    ...
    def process_gcode(self, gcode_stream):
        """
        THIS IS THE MAIN FUNCTION OF THE SCRIPT - Cura-specific G-code processing
        """
        from_gcode = GCodeLine.from_gcode #cache the lookup
        ...
        for line_number, line in enumerate(gcode_stream, start=1):
            ...
            #
            if still_in_header:
                if line.startswith(";TYPE:") or line.startswith(";LAYER:0"):
            ...
            # Detect Cura retraction settings
            self.detect_cura_retraction_settings(simulator, feature)
            ...
            # Detecting Speeds from the file (Cura-specific):
            if detect_speeds:
                if simulator.retraction_speed == 0 and simulator.is_retracting:
                    simulator.retraction_speed = simulator.f
                if simulator.wipe_speed == 0 and simulator.retraction_speed > 0:
                    simulator.wipe_speed = simulator.retraction_speed * 0.5  # Estimate wipe speed
            ...
            if layer_changed_during_internal_perimeter and simulator.is_extruding and simulator.is_moving:
                ...
                # and just continues after that, without any indication of ;TYPE:WALL-INNER
            ...
            if feature.internal_perimeter:
                ...
                if feature.layer >= start_at_layer and feature.layer not in layers_to_ignore: # Allows the processor to ignore certain layers
                    ...
                    ### Special commands that must be preserved
                    if myline.gcode.startswith(("SET_VELOCITY_LIMIT ", "M204 ")):
                        special_accel_command = myline # saves for later
                        continue  # skips this line


                    if not knife_activated and (simulator.retracted < 0 or simulator.just_stopped_extruding):
                        knife_activated = True
                    elif knife_activated and simulator.is_extruding and simulator.is_moving:
                        knife_activated = False
                    elif knife_activated and myline.gcode.startswith("G1 F"):
                        knife_activated = False

                    if  feature.justgotinside_internal_perimeter: # This should only execute once, when it reached ";TYPE:WALL-INNER"
            ...
            # Cura Layer-Change while not 'nesting' Internal Perimeters problem
            ...
            if feature.justleft_internal_perimeter or (layer_changed_during_internal_perimeter):
                ...
                if len(group_perimeter) > 0:
                    ...
                    if len(kept_loops) > 0:
                        ...
                        for loop_index, kept_loop in enumerate(kept_loops):
                            ...
                            for line_index, kept_line in enumerate(kept_loop):
                                ...
                                if is_first_loop and is_first_line:
                                    ...
                                    buffer_lines.append(from_gcode(";TYPE:WALL-INNER\n"))  # Cura internal perimeter type
                                    buffer_lines.append(from_gcode(f";HEIGHT:{feature.height}\n"))
                        ...
                        buffer_lines.append(from_gcode(f";HEIGHT:{feature.height:.2f}\n")) # For the Preview
            ...
            if feature.layer_change or (feature.current_type == "Custom" and feature.just_changed_type and current_state.z > 0):
                ...
                if feature.current_type == "external_perimeter":
                    ...
                    buffer_lines.append(from_gcode(";TYPE:WALL-OUTER\n"))  # Cura external perimeter type
            ...
            # Cura-specific preview fixes
            if feature.current_type in ["FILL", "SUPPORT", "SKIN"] and line.startswith(";HEIGHT:"):
                myline.gcode = f";HEIGHT:{feature.height}\n"
            if feature.just_changed_type and feature.current_type in ["FILL", "SUPPORT", "SKIN"]:
                buffer_lines.append(from_gcode(f";HEIGHT:{feature.height}\n"))

            # Exception for pretty visualization on Cura preview:
...
def main():
    ...
    parser = ErrorLoggingArgumentParser(
        description=f"""\
BrickLayers by Geek Detour - Cura Edition ({__version__})
Post-process Cura GCode for BrickLayers Z-shifting with extrusion adjustments.
https://github.com/GeekDetour/BrickLayers

This version is specifically designed for Cura G-code files.
...
""",
    ...
    parser.add_argument("input_file", 
                         help="\nPath to the input Cura G-code file\n\n")
